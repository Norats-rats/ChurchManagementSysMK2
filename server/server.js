...
app.post('/api/events/scan-qr', async (req, res) => {
  try {
    ...
    console.log("➡️ Restructured Scan Request incoming payload:", { body: req.body, query: req.query });
    ...
    } catch (e) { console.log("User lookup notice:", e.message); }
    ...
    if (existingLog) return res.status(400).json({ message: "You have already checked into this event today." });
    ...
    console.error("❌ Scan processing crash:", err);
...
app.post('/api/ai/analyze-metrics', async (req, res) => {
  try {
    ...
    // Direct proxy call out to Puter's official, high-stability AI portal route
    const httpResponse = await axios.post(
      'https://api.puter.com/puterai/openai/v1/chat/completions',
      {
        messages: [{ role: 'user', content: prompt }],
    ...
    // Remove markdown code fences if generated by the model anyway
    if (rawText.includes("```")) {
      const jsonMatch = rawText.match(/\{[\s\S]*\}/);
      if (jsonMatch) rawText = jsonMatch[0];
    }

    // Isolate pure bracket configurations to secure a clean parsing boundary
    const startBracket = rawText.indexOf('{');
    const endBracket = rawText.lastIndexOf('}');
    if (startBracket !== -1 && endBracket !== -1) {
      rawText = rawText.substring(startBracket, endBracket + 1);
    ...
    // Normalize properties cleanly so the frontend application receives flat string text
