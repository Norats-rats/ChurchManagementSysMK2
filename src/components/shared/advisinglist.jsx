...
const Advising = ({ user, role }) => {
  ...
  const styles = {
    ...
    buttonPrimary: { padding: '14px 24px', borderRadius: '14px', border: 'none', backgroundColor: '#2563eb', color: '#fff', fontWeight: '700', cursor: 'pointer' },
    buttonSecondary: { padding: '12px 20px', borderRadius: '14px', border: '1px solid #cbd5e1', background: '#fff', color: '#334155', cursor: 'pointer' },
    badge: (status) => ({ display: 'inline-flex', padding: '7px 12px', borderRadius: '999px', fontSize: '12px', fontWeight: '700', textTransform: 'uppercase', letterSpacing: '0.04em', backgroundColor: status === 'Accepted' ? '#dcfce7' : status === 'Archived' ? '#f8fafc' : '#eff6ff', color: status === 'Accepted' ? '#166534' : status === 'Archived' ? '#475569' : '#2563eb' }),
